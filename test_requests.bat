...
FOR /L %%A IN (1,1,1) DO curl -X POST -H "Content-Type: multipart/form-data;charset=ASCII" -F data="{\"email\":\"a.shcherbakov@ritoll.it\", \"name\":\"Mario Rossi %%A\", \"phone\":\"12345-%%A\", \"country\":\"Italia\",\"token\":\"super secret\"};type=application/json" -F file=@dumb.txt http://localhost/onm/register