...
function TActiveQueueModel.GetAvailablePath(const Path, Format: String): String;
var
  TryName: String;
...
var
  ...
  OutFile: String;
begin
  ConfigUpdated := TAQConfig.Create(FPort, Join(FListenersIPs, ','), Join(FProvidersIPs, ','), GetListeners());
  jo := Mapper.ObjectToJSONObject(ConfigUpdated);
  OutFile := GetAvailablePath(FFilePath, '-YYYY-mm-dd_hh_nn_ss');
  TFile.AppendAllText(OutFile, jo.ToString);
  jo.DisposeOf;
  ConfigUpdated.DisposeOf;
end;
